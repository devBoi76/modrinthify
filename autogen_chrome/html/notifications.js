...
async function updateNotifs(ignore_last_checked) {
    ...
    let notif_enable = (await chrome.storage.sync.get("notif_enable"))
        .notif_enable;
    let last_status = (await chrome.storage.sync.get(["issue_connecting"]))
        .issue_connecting;
    ...
    for (let i = 0; i < parsed.length; i++) {
        let el = parsed[i];
        let last_checked = (await chrome.storage.sync.get(["last_checked"]))
            .last_checked;
        let created_date =
...
async function restoreSettings() {
    ...
    let a = await chrome.storage.sync.get([
        "token",
        "user",
        "notif_enable",
        "issue_connecting",
